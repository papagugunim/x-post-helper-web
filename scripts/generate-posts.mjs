...
전반적으로 무겁고 딱딱한 분석보다는 가볍고 재미있게, 공감가는 방향을 우선한다.

[스포츠 뉴스 (Sports.ru, RPL, KHL 등)]
- 경기 결과나 선수 소식을 현지 팬 시각으로 생생하게
- 러시아 팬들의 반응이나 현장 분위기 묘사
- 한국 스포츠와 비교하며 공감 유발
- 유머나 아이러니 적극 활용
- 예: "KHL 플레이오프 첫 경기인데 러시아 친구들 오늘 다 사라짐 ㄷㄷ"

[연예/문화 뉴스 (StarHit, Светские хроники, 패션 채널 등)]
- 러시아 연예계 이슈를 한국인 감성으로 재해석
- 한/러 문화 차이에서 오는 재미있는 포인트 강조
- 가볍고 유머러스하게, 공감 위주
- 예: "불곰국 1위 연예인 이혼 발표났는데 댓글창이 한국이랑 똑같음 ㄷㄷ"

[한국 언론 러시아 뉴스 (Google News)]
- 한국 언론이 러시아를 어떻게 보는지 vs 실제 현지는 어떤지 대비
- "한국에선 이렇게 보도됐는데, 실제로 모스크바에서 보면..." 시각 추가
- 흥미롭거나 의외인 사실 위주로

[정치/경제 뉴스 (Interfax, RIA, Kommersant, RG 등)]
- 딱딱하게 분석하지 말고, 한국인이 공감할 수 있는 비유나 비교 활용
- 수치가 있으면 포함하되, 일상적인 언어로 풀어서
- 가능하면 아이러니나 유머 요소 넣기
---
...
async function main() {
  ...
  // 비율: 스포츠 4개, 연예/문화 4개, 구글뉴스 4개, 정치/경제 3개
  const pick = (arr, n) => arr.slice(0, n)
  const freshNewsRaw = [
    ...pick(sportsNews, 4),
    ...pick(entertainNews, 4),
    ...pick(googleNews, 4),
    ...pick(politicsNews, 3),
  ].slice(0, 15)
